...
void alterar(Lista *pplista, char s1[], float altura2){	
	struct No* pi = pesquisar(*pplista, s1);
	if(pi == NULL){
	printf("Pessoa nao encontrada\n");	
	}
	else{
		pi->dado.altura = altura2;
...
Lista ler_arquivo(){
	...
	FILE *file = fopen ("./imc_salvos.b", "rb");
	imc pplista;;
	while(fread(&pplista, sizeof(imc), 1, file)){
		inserir_fim(&lista, pplista);
	}
	fclose(file);
	return lista;
...
void salvar_arquivo(Lista lista){
	FILE *file = fopen ("./imc_salvos.b", "wb");
	struct No* pi;
	for(pi = lista.inicio; pi != NULL ; pi = pi->prox){
		fwrite(&pi->dado, sizeof(imc), 1, file);
	}
	fclose(file);
}
void menu(){
	...
	printf("|14 - Alterar dados de uma pessoa       |\n");
...
int main(){
	...
	while(op != 19){
		if(op == 1){
		    ...
		    char nome2[100];
			printf("Informe a pessoa\n");
			scanf("%s", nome2);
			float altura2;
			printf("Informe a nova altura\n");
			scanf("%f", &altura2);
			alterar(&lista, nome2, altura2);
			printf("\n");
		}
		else if(op == 15){
